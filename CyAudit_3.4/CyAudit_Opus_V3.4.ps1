...
<#
.SYNOPSIS
    CyAudit Opus v3.4 - Advanced Windows Security Configuration Audit Tool with PowerSTIG Integration
...
.DESCRIPTION
    ...
    Version 3.4 integrates Microsoft PowerSTIG for comprehensive DISA STIG compliance while
...
.EXAMPLE
    .\CyAudit_Opus_V3.4.ps1 -ClientName "Acme Corp"
    # Runs both V3.3 and PowerSTIG evaluations with side-by-side comparison

.EXAMPLE
    .\CyAudit_Opus_V3.4.ps1 -ComputerName "SERVER01","SERVER02" -ClientName "Acme Corp" -Credential (Get-Credential)
    # Remote audit with both evaluation methods

.EXAMPLE
    .\CyAudit_Opus_V3.4.ps1 -ClientName "Acme Corp" -GenerateCheckList -XccdfPath "C:\STIGs"
    # Full audit with STIG Viewer checklist generation

.EXAMPLE
    .\CyAudit_Opus_V3.4.ps1 -ClientName "Acme Corp" -PowerSTIGOnly
    # Run only PowerSTIG evaluation (skip V3.3 checks)
...
# Script version
$ScriptVersion = "CyAudit Opus PowerShell v3.4.0"
...
function Test-PowerSTIGPrerequisites {
    ...
    Write-AuditLog "✓ $($psVersionCheck.Message)" -LogFile $LogFile
    ...
    Write-AuditLog "✓ PowerSTIG module found (Version: $($powerSTIG.Version))" -LogFile $LogFile
    ...
    foreach ($moduleName in $requiredModules) {
        ...
        if ($module) {
            Write-AuditLog "✓ $moduleName found (Version: $($module.Version))" -LogFile $LogFile
        } else {
            Write-AuditLog "✗ $moduleName not found" -LogFile $LogFile
...
function Install-PowerSTIGModules {
    ...
    try {
        ...
        Write-AuditLog "✓ PowerSTIG installed successfully" -LogFile $LogFile
        ...
        foreach ($module in $dependencies) {
            ...
            try {
                ...
                Write-AuditLog "✓ $module installed" -LogFile $LogFile
            } catch {
...
function New-PowerSTIGConfiguration {
    ...
    try {
        ...
        # Determine STIG version (use latest if not specified)
        $stigVersion = if ($StigVersions -and $StigVersions.Count -gt 0) {
            $StigVersions[0]
        } else {
            # Map OS to latest known STIG version (PowerSTIG 4.24.0)
            $versionMap = @{
                'WindowsClient-11' = '2.2'        # Windows 11 STIG Ver 2, Rel 2 (V2R2)
                'WindowsClient-10' = '3.2'        # Windows 10 STIG Ver 3, Rel 2 (V3R2)
                'WindowsServer-2022-DC' = '2.2'   # Server 2022 STIG Ver 2, Rel 2 (V2R2)
                'WindowsServer-2022-MS' = '2.2'   # Server 2022 STIG Ver 2, Rel 2 (V2R2)
                'WindowsServer-2019-DC' = '3.2'   # Server 2019 STIG Ver 3, Rel 2 (V3R2)
                'WindowsServer-2019-MS' = '3.2'   # Server 2019 STIG Ver 3, Rel 2 (V3R2)
            }

            $key = "$($stigInfo.STIGType)-$($stigInfo.OSVersion)"
            if ($stigInfo.OSRole -and $stigInfo.STIGType -eq 'WindowsServer') {
                $key += "-$($stigInfo.OSRole)"
            }

            if ($versionMap.ContainsKey($key)) {
                $version = $versionMap[$key]
                Write-AuditLog "Using default STIG version: $version for $key" -LogFile $LogFile
                $version
            } else {
                Write-AuditLog "ERROR: No default STIG version defined for $key" -LogFile $LogFile
                throw "Unsupported OS/Role combination: $key"
            }
        }

        # Validate STIG version exists in PowerSTIG module
        ...
        if (-not (Test-Path $expectedXmlFile)) {
            ...
            Get-ChildItem -Path $stigDataPath -Filter "$($stigInfo.STIGType)-$($stigInfo.OSVersion)-*.xml" |
...
"@
        ...
        if (Test-Path $mofPath) {
            ...
            Write-AuditLog "✓ MOF file generated successfully ($mofSize bytes)" -LogFile $LogFile
...
function Test-WinRMAvailability {
    ...
    try {
        ...
        Write-AuditLog "✓ WinRM is available for localhost DSC operations" -LogFile $LogFile
        return $true
...
function New-STIGViewerChecklist {
    ...
    try {
        ...
        Write-AuditLog "✓ STIG Viewer checklist created: $cklPath" -LogFile $LogFile
        return $cklPath
...
function Export-EnhancedSTIGReport {
    ...
    $summary = [PSCustomObject]@{
        ...
        BothCompliant = ($ComparisonResults | Where-Object { $_.Comparison -eq '✓ Both Compliant' }).Count
        BothNonCompliant = ($ComparisonResults | Where-Object { $_.Comparison -eq '✗ Both Non-Compliant' }).Count
                    ...
                    if ($configResult -and $configResult.Success) {
                        ...
                        if ($powerSTIGAuditResult.Success) {
                            ...
                            if ($GenerateCheckList -and $XccdfPath) {
                                ...
                                if ($cklPath) {
                                    Write-Host "✓ STIG Viewer checklist created: $cklPath" -ForegroundColor Green
                                }
                            }
                            ...
                            if ($stigResults -and $powerSTIGResults) {
                                ...
                                Write-Host "  Both Compliant: $(($comparisonResults | Where-Object { $_.Comparison -eq '✓ Both Compliant' }).Count)" -ForegroundColor Green
                                Write-Host "  Both Non-Compliant: $(($comparisonResults | Where-Object { $_.Comparison -eq '✗ Both Non-Compliant' }).Count)" -ForegroundColor Red
                            ...
                            Write-Host "✓ PowerSTIG audit completed" -ForegroundColor Green
