...
This integration transforms CyAudit Opus v3.4 security assessment output into Splunk Cloud-optimized newline-delimited JSON (NDJSON) format for efficient ingestion and analysis.
...
│ CyAudit_Opus_V3.4.ps1 → 52 files (CSV/JSON/XML/TXT)           │
│ - UTF-16 LE encoded                                             │
...
1. **CyAudit Opus v3.4** installed and configured
...
```powershell
.\CyAudit_Opus_V3.4.ps1 -ClientName "MyCompany" -ComputerName "SERVER01"
```
...
.\CyAudit_Opus_V3.4.ps1 -ClientName "Acme Corp" -ComputerName "WEB-SERVER-01"
...
foreach ($system in $systems) {
    # Run assessment
    .\CyAudit_Opus_V3.4.ps1 -ClientName "Acme Corp" -ComputerName $system
...
foreach ($system in $Systems) {
    try {
        # Assessment
        .\CyAudit_Opus_V3.4.ps1 -ClientName "Weekly Assessment" -ComputerName $system -ErrorAction Stop
...
**Solutions**:
1. Check input files are from CyAudit v3.4 (not earlier versions)
...
This integration is provided as-is for use with CyAudit Opus v3.4 and Splunk Cloud.
