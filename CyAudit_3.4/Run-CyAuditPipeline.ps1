...
.DESCRIPTION
    ...
    1. Runs CyAudit_Opus_V3.4.ps1 security assessment
...
.NOTES
    ...
    - CyAudit_Opus_V3.4.ps1 in same directory
...
try {
    ...
    $cyauditScript = Join-Path $ScriptDir "CyAudit_Opus_V3.4.ps1"
    ...
    if ($script:Config.CyAuditOptions) {
        if ($script:Config.CyAuditOptions.OutputPath) {
    ...
    try {
        ...
        # Find the output directory (most recent directory in OutputBasePath)
        $assessmentDir = Get-ChildItem -Path $script:Config.OutputBasePath -Directory |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 1

        if (-not $assessmentDir) {
            throw "Could not locate CyAudit output directory in $($script:Config.OutputBasePath)"
        }
