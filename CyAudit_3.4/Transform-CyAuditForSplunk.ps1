...
<#
.SYNOPSIS
    Transform CyAudit Opus v3.4 output for Splunk Cloud ingestion
...
function Write-SplunkNDJson {
    ...
    try {
        ...
        # Write each object as a single line of JSON (NDJSON format)
        $streamWriter = [System.IO.StreamWriter]::new($OutputFile, $false, [System.Text.Encoding]::UTF8)

        foreach ($obj in $Objects) {
            # Convert to JSON (compact, no indentation)
        ...
        Write-Verbose "Wrote $($Objects.Count) events to $OutputFile"
        return $true

    } catch {
        Write-TransformLog "Failed to write NDJSON: $OutputFile - $($_.Exception.Message)" -Level Error
...
function Transform-ErrorLog {
    ...
    try {
        # ErrorLog.txt is UTF-8, not UTF-16
...
try {
    ...
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_systeminfo.json")
    }

    $data = Transform-WindowsVersionInfo -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_versioninfo.json")
    }

    Write-Host ""
    Write-Host "───────────────────────────────────────────────────────────────" -ForegroundColor Cyan
    Write-Host " Phase 2: Identity & Access Management" -ForegroundColor Cyan
    Write-Host "───────────────────────────────────────────────────────────────" -ForegroundColor Cyan

    # Transform Users and Groups
    $data = Transform-Users -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_users.json")
    }

    $data = Transform-Groups -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_groups.json")
    }
    ...
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_passwordpolicies.json")
    }

    $data = Transform-AuditPolicies -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_auditpolicies.json")
    }

    $data = Transform-UserRights -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_userrights.json")
    }

    Write-Host ""
    Write-Host "───────────────────────────────────────────────────────────────" -ForegroundColor Cyan
    Write-Host " Phase 4: Registry & Configuration" -ForegroundColor Cyan
    Write-Host "───────────────────────────────────────────────────────────────" -ForegroundColor Cyan

    # Transform Registry
    $data = Transform-RegistryValues -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_registry.json")
    }

    $data = Transform-STIGRegistryCompliance -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_stig_registry.json")
    }

    Write-Host ""
    Write-Host "───────────────────────────────────────────────────────────────" -ForegroundColor Cyan
    Write-Host " Phase 5: Services & Features" -ForegroundColor Cyan
    Write-Host "───────────────────────────────────────────────────────────────" -ForegroundColor Cyan

    # Transform Services and Features
    $data = Transform-Services -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_services.json")
    }

    $data = Transform-WindowsFeatures -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_features.json")
    }
    ...
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_filepermissions.json")
    }

    $data = Transform-DirectoryPermissions -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_dirpermissions.json")
    }

    Write-Host ""
    Write-Host "───────────────────────────────────────────────────────────────" -ForegroundColor Cyan
    Write-Host " Phase 7: Storage & Patching" -ForegroundColor Cyan
    Write-Host "───────────────────────────────────────────────────────────────" -ForegroundColor Cyan

    # Transform Storage and Patching
    $data = Transform-Drives -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_drives.json")
    }

    $data = Transform-HotFixes -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_hotfixes.json")
    }

    $data = Transform-MissingHotfixes -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_missinghotfixes.json")
    }

    Write-Host ""
    Write-Host "───────────────────────────────────────────────────────────────" -ForegroundColor Cyan
    Write-Host " Phase 8: Event Logs & Network" -ForegroundColor Cyan
    Write-Host "───────────────────────────────────────────────────────────────" -ForegroundColor Cyan

    # Transform Event Logs
    $data = Transform-LogSettings -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_logsettings.json")
    }

    # Transform Network
    $data = Transform-Shares -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_shares.json")
    }
    ...
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_stig_summary_v33.json")
    }

    $data = Transform-PowerSTIGFindings -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_stig_powerstig.json")
    }

    $data = Transform-STIGComparison -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_stig_comparison.json")
    }

    $data = Transform-STIGMergedResults -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_stig_merged.json")
    }
    ...
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_enhanced_summary.json")
    }

    $data = Transform-ExecutiveSummary -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_executive_summary.json")
    }

    # Transform Error Log
    $data = Transform-ErrorLog -InputPath $InputPath -ComputerName $computerName -AuditDate $auditDate
    if ($data) {
        Write-SplunkNDJson -Objects $data -OutputFile (Join-Path $OutputPath "cyaudit_errorlog.json")
    }
