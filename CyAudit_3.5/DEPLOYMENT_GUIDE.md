# CyAudit 3.5 - Complete Deployment and Scheduling Guide
...
This guide provides complete instructions for deploying the CyAudit 3.5 automated security assessment and Splunk integration pipeline. The solution automates:

1. **Security Assessment** - CyAudit Opus v3.5 performs comprehensive Windows security audits
...
```
C:\CyAudit\
├── CyAudit_3.5\                          # Application files (read-only)
│   ├── CyAudit_Opus_V3.5.ps1             # Security assessment engine
...
1. Copy the entire `CyAudit_3.5` folder to your target location:
   ```
   Recommended: C:\CyAudit\CyAudit_3.5\
   ```

2. Verify all files extracted successfully:
   ```powershell
   Get-ChildItem -Path "C:\CyAudit\CyAudit_3.5" -Recurse | Measure-Object | Select-Object -ExpandProperty Count
...
d-----        11/12/2025  12:00 PM                CyAudit_3.5
...
1. Open the configuration file:
   ```powershell
   notepad "C:\CyAudit\CyAudit_3.5\CyAuditPipeline.config.json"
   ```
...
2025-11-12 14:30:00 [INFO] Loading configuration from: C:\CyAudit\CyAudit_3.5\CyAuditPipeline.config.json
...
- **Add arguments**:
  ...
  ```
- **Start in**: `C:\CyAudit\CyAudit_3.5`
...
$ScriptPath = "C:\CyAudit\CyAudit_3.5\Run-CyAuditPipeline.ps1"
$WorkingDirectory = "C:\CyAudit\CyAudit_3.5"
...
4. **Working Directory Not Set**
   - Task Scheduler: Ensure "Start in" field is set to `C:\CyAudit\CyAudit_3.5`
...
```powershell
# Verify file exists
Test-Path "C:\CyAudit\CyAudit_3.5\CyAuditPipeline.config.json"
...
1. Run CyAudit manually to see detailed error:
   ```powershell
   cd "C:\CyAudit\CyAudit_3.5"
   .\CyAudit_Opus_V3.5.ps1 -ComputerName "localhost" -ClientName "Test"
   ```
...
1. Verify email configuration:
   ```powershell
   $config = Get-Content "C:\CyAudit\CyAudit_3.5\CyAuditPipeline.config.json" | ConvertFrom-Json
   $config.EmailAlerts
   ```
...
1. **Create separate configuration files** for each system:
   ```
   C:\CyAudit\CyAudit_3.5\config_server01.json
   C:\CyAudit\CyAudit_3.5\config_server02.json
   ```
...
1. **Reduce CyAudit scope**: Edit `CyAudit_Opus_V3.5.ps1` to skip non-essential checks
...
### Initial Setup
- [ ] Extract files to `C:\CyAudit\CyAudit_3.5\`
