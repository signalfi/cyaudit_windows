...
try {
    ...
    foreach ($file in $jsonFiles) {
        ...
        if ($jsonTest.Valid) {
            Write-ValidationLog "    [OK] All $($jsonTest.LineCount) events have valid JSON syntax" -Level Success
            $testsPassed++
        } else {
            Write-ValidationLog "    [X] JSON syntax errors found:" -Level Error
        ...
        if ($encodingTest.Valid) {
            Write-ValidationLog "    [OK] Encoding: $($encodingTest.Encoding)" -Level Success
            $testsPassed++
        } else {
            Write-ValidationLog "    [X] Invalid encoding: $($encodingTest.Encoding)" -Level Error
            $testsFailed++
        }
        ...
        if ($fieldsTest.Valid) {
            Write-ValidationLog "    [OK] All required fields present" -Level Success
            $testsPassed++
        } else {
            Write-ValidationLog "    [X] Missing required fields:" -Level Error
        ...
        if ($timestampTest.Valid) {
            Write-ValidationLog "    [OK] Timestamps are ISO 8601 compliant" -Level Success
            $testsPassed++
        } else {
            Write-ValidationLog "    [X] Invalid timestamp formats:" -Level Error
        ...
        if ($testsFailed -eq 0) {
            Write-ValidationLog "  [OK] ALL TESTS PASSED ($testsPassed/$testsPassed)" -Level Success
            $filesPassed++
        } else {
            Write-ValidationLog "  [X] SOME TESTS FAILED ($testsPassed passed, $testsFailed failed)" -Level Error
            $filesFailed++
        }
    ...
    $allStats | Group-Object -Property Sourcetype | Sort-Object Count -Descending | ForEach-Object {
        $eventTotal = ($_.Group | Measure-Object -Property EventCount -Sum).Sum
        Write-ValidationLog "  $($_.Name): $eventTotal events" -Level Info
    }
    ...
    Write-Host ""
    if ($filesFailed -eq 0) {
        Write-ValidationLog "[OK] All files passed validation! Data is ready for Splunk." -Level Success
        Write-Host ""
        Write-ValidationLog "Next step: Upload to Splunk Cloud using Upload-ToSplunkCloud.ps1" -Level Info
        exit 0
    } else {
        Write-ValidationLog "[X] Validation completed with errors. Please review and fix issues." -Level Error
        exit 1
    }
